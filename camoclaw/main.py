...
# Add project root to path so "camoclaw" package can be imported
_camoclaw_dir = Path(__file__).resolve().parent
project_root = _camoclaw_dir.parent
...
def load_config(config_path: str) -> dict:
    ...
    # Windows default encoding may be GBK; configs often include UTF-8 (e.g., Chinese prompts).
    # Prefer UTF-8, then UTF-8 with BOM, then fall back to platform default.
...
async def main(config_path: str, exhaust: bool = False):
    ...
    lb_config = config.get("camoclaw") or config.get("livebench")
    ...
    if exhaust:
        ...
        # 明确这一段时间范围是「本次运行所用配置文件」里的 date_range，
        # 方便区分默认 config 和演化 / Run2 生成的临时 config。
        cfg_name = Path(config_path).name
        print(f"📅 Config Date Range ({cfg_name}): {init_date} to {end_date}")
    ...
    for agent_config in enabled_agents:
        ...
        agent = LiveAgent(
            ...
            data_path=os.path.join(
                lb_config.get("data_path", "./camoclaw/data/agent_data"),
            ...
            api_timeout=float(lb_config["agent_params"].get("api_timeout", 120)),
            ...
            # Pass skill module (optional)
            skill_enabled=lb_config.get("skill", {}).get("enabled", False),
            # When True (default), load both global built-in and agent skills
            skill_use_builtin=lb_config.get("skill", {}).get("use_builtin", True),
            # Optional: force daily activity ("work" or "learn")
            force_activity=lb_config.get("agent_params", {}).get("force_activity"),
        )
        # Plan A integration: optional evolution hook (Learn + Run2, promote to main agent store)
        agent.evolution_config = lb_config.get("evolution", {})
        agent.evolution_config_path = config_path
    ...
    print(f"\n{'='*60}")
    print("🏁 CAMOCLAW SIMULATION COMPLETE")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "config",
        nargs="?",
        default="camoclaw/configs/default_config.json",
        help="Path to configuration file (default: camoclaw/configs/default_config.json)"
    )
    parser.add_argument(
        "--exhaust",
        action="store_true",
