...
from camoclaw.utils.logger import LiveBenchLogger, set_global_logger
...
class LiveAgent:
    ...
    def __init__(
        ...
        # Set data path
        self.data_path = data_path or f"./camoclaw/data/agent_data/{signature}"
    ...
    async def initialize(self) -> None:
        ...
        from camoclaw.tools.direct_tools import get_all_tools, set_global_state as set_tool_state
        ...
        for idx, candidate_path in enumerate(ref_file_paths):
            ...
            if exists_after:
                ...
                try:
                    ...
                    try:
                        from camoclaw.tools.productivity.code_execution_sandbox import upload_task_reference_files
    ...
    async def run_daily_session(self, date: str) -> Optional[str]:
        ...
        try:
            from camoclaw.tools.direct_tools import set_global_state as set_tool_state
            set_tool_state(
                signature=self.signature,
                economic_tracker=self.economic_tracker,
                task_manager=self.task_manager,
        ...
        if self.skill_enabled:
            try:
                from camoclaw.skill import get_skill_prompt_section
        ...
        if not activity_completed and self.current_task:
            ...
            try:
                from camoclaw.agent.wrapup_workflow import create_wrapup_workflow
        ...
        try:
            from camoclaw.tools.productivity.code_execution_sandbox import SessionSandbox
        ...
        try:
            from camoclaw.tools.productivity.code_execution_sandbox import cleanup_session_sandbox
