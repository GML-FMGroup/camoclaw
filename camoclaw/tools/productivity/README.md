...
├── code_execution_sandbox.py # Code execution (E2B sandbox)
├── file_reading.py          # File reading (PDF/DOCX/XLSX etc.)
...
### 3. execute_code_sandbox (code_execution_sandbox.py)
- **Purpose**: Execute Python code in E2B cloud sandbox
- **Dependencies**: e2b-code-interpreter
- **Output**: stdout, stderr, exit code, downloaded_artifacts
...
5. Update `direct_tools.py` get_all_tools():
   ```python
   from camoclaw.tools.productivity import (..., your_tool)
