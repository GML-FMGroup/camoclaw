...
def read_file(filetype: str, file_path: Path) -> Dict[str, Any]:
    ...
    Returns:
        ...
        If file is missing, returns {"error": "File not found", "path": ..., "hint": ...}.
    """
    # Normalize path (str or Path, forward/back slashes) and check existence
    path = Path(file_path) if not isinstance(file_path, Path) else file_path
    if not path.is_absolute():
        path = path.resolve()
    # Windows: use long path form so open()/exists() succeed when path >= 260 chars
    try:
        from camoclaw.utils.path_io import path_for_io
        path_io = path_for_io(str(path))
        if path_io:
            path = Path(path_io)
    except Exception:
        pass
    if not path.exists():
        return {
            ...
            "path": str(path),
            "hint": "Reference files for this task may not be available (dataset often does not include asset files). Proceed using only the task description and your best judgment."
        }
    ...
    if filetype == "pdf":
        ...
        if supports_multimodal:
            # Use image-based approach for multimodal models (requires poppler on host)
            ...
            # Fallback: poppler not installed (e.g. Windows) or conversion failed — extract text with PyPDF2
            print(f"📄 PDF images unavailable (poppler missing?); falling back to PyPDF2 text extraction")
            text = read_pdf_text_pypdf2(path)
            return {
                "type": "text",
                "text": text or "(No text extracted from PDF)",
                "message": "PDF read as text (poppler not available; use this text for the task)."
            }
        else:
        ...
        images = read_pptx_as_images(path)
        if images:
            return {
                "type": "pptx_images",
                "images": images,
        ...
        print(f"📊 PPTX images unavailable (LibreOffice missing?); falling back to python-pptx text extraction")
        text = read_pptx_text(path)
        return {
            "type": "text",
            "text": text or "(No text extracted from PPTX)",
            "message": "PPTX read as text (LibreOffice not available; use this text for the task)."
        }
    
    elif filetype in ["png", "jpg", "jpeg"]:
        print(f"🖼️  Reading {filetype.upper()} via read_image()")
        image_data = read_image(path, filetype)
        return {
            "type": "image",
...
def read_pptx_text(pptx_path: Path) -> str:
    """
    Extract text from PPTX using python-pptx only (no LibreOffice; works on Windows).
    Use as fallback when read_pptx_as_images fails.
    """
    if not os.path.exists(pptx_path):
        raise FileNotFoundError(f"PPTX file not found: {pptx_path}")
    try:
...
def read_pdf_as_images(pdf_path: Path) -> Optional[List[bytes]]:
    ...
    try:
        # Optional: use custom poppler path (e.g. Windows: set POPPLER_PATH to .../Library/bin)
        ...
        images = convert_from_path(str(pdf_path), dpi=100, poppler_path=poppler_path)
...
def read_pdf_text_pypdf2(pdf_path: Path) -> str:
    """
    Extract text from PDF using PyPDF2 only (no poppler, works on Windows).
    Use as fallback when read_pdf_as_images fails (e.g. poppler not installed).
    """
    if not os.path.exists(pdf_path):
        raise FileNotFoundError(f"PDF file not found: {pdf_path}")
...
if __name__ == "__main__":
    pdf = Path(
        "/Users/tianyu/Desktop/workspace/-Live-Bench/camoclaw/data/test_data/AITrader.pdf"
    )
